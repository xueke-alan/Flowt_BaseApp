<template>
  <div id="main-view-qiankun-contener" :class="{ hide: !isQiankunRouter }">
    <page100 class="cover" v-if="qiankunBus.loading" />
    <div
      :id="'main-view-qiankun-' + r"
      v-for="r in qiankunRoutersNameList"
      v-show="showContener(r)"
    >
      <!-- TODO 乾坤加载完毕之后才卸载遮罩。 -->
      <!-- 数据库显示已开发却没有上限 -->
    </div>
...
<script lang="ts" setup>
  ...
  const showContener = (r) => {
    return sigleQiankunContainer || nowRouter.value.fullPath.indexOf(r) > 0;
  };
</script>
